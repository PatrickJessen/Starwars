...
namespace Starwars
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Opg1
            List<Planet> namesWithM = planets.Where(s => s.Name.StartsWith("M")).ToList();
            //Opg2
            List<Planet> namesContainsY = planets.Where(s => s.Name.ToLower().Contains("y")).ToList();
            //Opg3
            List<Planet> planetsWith9Signs = planets.Where(s => s.Name.Length > 9 && s.Name.Length < 15).ToList();
            //Opg4
            List<Planet> planetsWithA2EndsWE = planets.Where(s => s.Name.ToLower()[1] == 'a' && s.Name.ToLower()[s.Name.Length -1] == 'e').ToList();
            //Opg5
            var planetsWithRotationGreaterThan40 = from s in planets where s.RotationPeriod > 40 orderby s.RotationPeriod select s;
            //Opg6
            var opg6 = from s in planets where s.RotationPeriod > 10 && s.RotationPeriod < 20 orderby s.Name select s;
            //Opg7
            var opg7 = from s in planets where s.RotationPeriod > 30 orderby s.Name orderby s.RotationPeriod select s;
            //Opg8
            var opg8 = from s in planets where s.RotationPeriod < 30 || s.SurfaceWater > 50 && s.Name.ToLower().Contains("ba") orderby s.Name orderby s.RotationPeriod orderby s.SurfaceWater select s;
            //Opg9
            var opg9 = from s in planets where s.SurfaceWater > 0 orderby s.SurfaceWater descending select s;
            //Opg10
            var opg10 = from s in planets orderby s.Diameter * s.Population ascending select s; // fuck math
            //Opg11
            planets.RemoveAll(s => s.RotationPeriod > 0);
            //opg12
            var opg12 = planets.Where(s => s.Name.ToLower().StartsWith("a") || s.Name.ToLower().StartsWith("s")).ToList();
            var opg12two = planets.Where(s => s.Terrain == s.Terrain.Where(f => f == "rainforest")).ToList();
            var list = opg12.Union(opg12two).ToList();
            //Opg13
            var opg13 = planets.Where(s => s.Terrain == s.Terrain.Where(f => f == "deserts")).ToList();
            //Opg14
            var opg14 = planets.Where(s => s.Terrain == s.Terrain.Where(f => f == "swaps")).OrderBy(s => s.RotationPeriod).OrderBy(s => s.Name).ToList();
            //Opg15
            var opg15 = planets.Select(s => Regex.Split(s.Name, @"(?=[aeiou])").Where(s => !string.IsNullOrWhiteSpace(s)).OrderBy(n => n).Aggregate((a, b) => a + b));
            //Opg16
            string[] strs = new string[] { "kk", "ll", "rr", "nn"};
            var opg16 = planets.Where(s => s.Name.ToLower().Contains(strs.Select(f => f).First())).OrderByDescending(s => s.Name).ToList();
            Console.ReadKey();
        }
